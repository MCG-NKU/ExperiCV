        ...
        with torch.no_grad():
            ...
            for i, (images, labels) in enumerate(loader):
                ...
                for kk in range(outputs.shape[0]):
                    pre_cls = (outputs[kk].unsqueeze(0).max(dim=1)[1].cpu().numpy())
                    pred_mask = label2color(pre_cls)[0].astype(np.uint8)
                    Image.fromarray(pred_mask).save(f"./results/_{self.step}.png")
                    print(labels.shape, pre_cls.shape)
                    labels = label2color(labels[kk].cpu().numpy())
                    Image.fromarray(labels).save(f"./results/_{self.step}.jpg")
                    break
                break
